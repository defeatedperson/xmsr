...
async function runVideoPipeline(opts, controller) {
  ...
  try {
    ...
    if (interp?.enabled) {
      ...
      onLog(`阶段3/4：插帧（目标 ${interp.targetFps}fps，约 ${mult}x）`)
    ...
    const outName = `${basename(videoPath).replace(/\.[^.]+$/, '')}_x${superRes.scale}${interp?.enabled ? `_${interp.targetFps}fps` : ''}.mp4`
