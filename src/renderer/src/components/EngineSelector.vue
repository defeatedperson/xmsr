...
function onEngineChange(id) {
  ...
  if (eng) {
    // 模型默认选第一个
    model.value = eng.models[0]?.name || ''
    // 倍率默认选第一个支持的（由 watch model 校正）
    const firstScale = eng.models[0]?.scales?.[0]
    if (firstScale) scale.value = firstScale
  }
}

// 切换模型时，若当前倍率不被新模型支持，则校正到第一个支持的倍率
watch(model, () => {
  if (currentModel.value && !currentModel.value.scales.includes(scale.value)) {
    scale.value = currentModel.value.scales[0]
  }
})
