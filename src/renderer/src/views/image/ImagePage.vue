  ...
  <div class="image-page">
    ...
    <div class="main-layout">
      ...
      <el-card class="right-panel" shadow="never">
        ...
        <ParamPanel
          v-model="paramConfig"
          :engine-id="engineConfig.engineId"
          :supports-denoise="engineConfig.supportsDenoise"
          :denoise-levels="allowedDenoiseLevels"
        />
...
const paramConfig = ref({ denoise: 1, format: 'png' })
...
// 当前 引擎+模型+倍率 实际允许的降噪等级：
// 优先取模型按倍率扫描出的等级（如 realcugan models-nose 仅 -1、models-se 3x/4x 仅 -1/0/3），
// 再与引擎声明的等级取交集；模型无该信息时回退到引擎级列表
const allowedDenoiseLevels = computed(() => {
  const cfg = engineConfig.value
  if (!cfg.supportsDenoise) return []
  const engineLevels = cfg.denoiseLevels || []
  const modelLevels = cfg.denoiseByScale?.[cfg.scale]
  if (Array.isArray(modelLevels) && modelLevels.length) {
    const filtered = modelLevels.filter((lv) => engineLevels.includes(lv))
